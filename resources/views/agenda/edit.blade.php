<x-app-layout>
    <x-slot name="header">Edit Agenda Kegiatan</x-slot>

    <div class="max-w-4xl mx-auto">
        <form action="{{ route('agenda.update', $agenda->id) }}" method="POST" class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            @csrf
            @method('PUT')
            
            <div class="px-8 py-5 bg-white border-b border-slate-100">
                <h2 class="text-slate-800 font-bold text-xl">Edit Data Agenda</h2>
                <p class="text-slate-500 text-sm mt-1">Perbarui informasi agenda kegiatan di bawah ini.</p>
            </div>
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama_kegiatan" value="{{ old('nama_kegiatan', $agenda->nama_kegiatan) }}" 
                        class="w-full rounded-lg py-3 px-4 shadow-sm text-slate-700
                        @error('nama_kegiatan') border-red-500 focus:ring-red-500 focus:border-red-500 @else border-slate-300 focus:ring-blue-500 focus:border-blue-500 @enderror" required>
                ...
                <div>
                    ...
                    <select name="jenis_agenda_id" class="w-full rounded-lg py-3 px-4 shadow-sm text-slate-700 bg-white
                        ...
                        @foreach($jenisAgendas as $jenis)
                            <option value="{{ $jenis->id }}" {{ old('jenis_agenda_id', $agenda->jenis_agenda_id) == $jenis->id ? 'selected' : '' }}>
                                {{ $jenis->nama_jenis }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="waktu_mulai" value="{{ old('waktu_mulai', date('Y-m-d\TH:i', strtotime($agenda->waktu_mulai))) }}" 
                        class="w-full rounded-lg py-3 px-4 shadow-sm text-slate-700
                ...
                <div>
                    ...
                    <input type="datetime-local" name="waktu_selesai" value="{{ old('waktu_selesai', date('Y-m-d\TH:i', strtotime($agenda->waktu_selesai))) }}" 
                        class="w-full rounded-lg py-3 px-4 shadow-sm text-slate-700
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="tempat" value="{{ old('tempat', $agenda->tempat) }}" 
                        class="w-full rounded-lg py-3 px-4 shadow-sm text-slate-700
                        @error('tempat') border-red-500 focus:ring-red-500 focus:border-red-500 @else border-slate-300 focus:ring-blue-500 focus:border-blue-500 @enderror" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="3" 
                        class="w-full rounded-lg py-3 px-4 shadow-sm text-slate-700
                        @error('keterangan') border-red-500 focus:ring-red-500 focus:border-red-500 @else border-slate-300 focus:ring-blue-500 focus:border-blue-500 @enderror">{{ old('keterangan', $agenda->keterangan) }}</textarea>
                </div>
                        ...
                        <div>
                            ...
                            <select name="surat_masuk_id" class="w-full rounded-lg py-2 px-3 text-sm shadow-sm text-slate-700 border-slate-300 focus:ring-blue-500 focus:border-blue-500 bg-white">
                                ...
                                @foreach($suratMasuks as $sm)
                                    <option value="{{ $sm->id }}" {{ old('surat_masuk_id', $agenda->surat_masuk_id) == $sm->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="surat_keluar_id" class="w-full rounded-lg py-2 px-3 text-sm shadow-sm text-slate-700 border-slate-300 focus:ring-blue-500 focus:border-blue-500 bg-white">
                                ...
                                @foreach($suratKeluars as $sk)
                                    <option value="{{ $sk->id }}" {{ old('surat_keluar_id', $agenda->surat_keluar_id) == $sk->id ? 'selected' : '' }}>
