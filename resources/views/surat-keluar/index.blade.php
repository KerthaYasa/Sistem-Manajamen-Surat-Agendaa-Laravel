<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        ...
        <form action="{{ route('surat-keluar.index') }}" method="GET" class="w-full md:w-auto flex flex-col md:flex-row gap-2">
            ...
            <div class="relative w-full md:w-64">
                ...
                <a href="{{ route('surat-keluar.index') }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-red-500" title="Reset Pencarian">
        ...
        {{-- Tombol Tambah --}}
        <a href="{{ route('surat-keluar.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-medium text-sm text-white hover:bg-blue-700 transition shadow-sm">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-slate-600">
                <thead class="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-200">
                    <tr>
                        <th class="px-6 py-3 whitespace-nowrap">No. Agenda/Surat</th>
                        <th class="px-6 py-3 whitespace-nowrap">Tgl. Surat</th>
                        <th class="px-6 py-3 whitespace-nowrap">Kategori</th>
                        <th class="px-6 py-3 whitespace-nowrap">Tujuan</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-6 py-3 font-medium text-slate-900 whitespace-nowrap">
                            {{ $surat->nomor_surat }}
                            <div class="text-xs text-slate-400 mt-0.5">Agenda: {{ $surat->nomor_agenda }}</div>
                        </td>
                        ...
                        <td class="px-6 py-3 whitespace-nowrap">
                            @if($surat->status == 'Draft')
                                <span class="bg-slate-100 text-slate-600 px-2 py-1 rounded-full text-xs font-semibold">Draft</span>
                            @elseif($surat->status == 'Dikirim')
                                <span class="bg-blue-100 text-blue-600 px-2 py-1 rounded-full text-xs font-semibold">Dikirim</span>
                            @elseif($surat->status == 'Diarsipkan')
                                <span class="bg-green-100 text-green-600 px-2 py-1 rounded-full text-xs font-semibold">Diarsipkan</span>
                            @else
                                <span class="bg-gray-100 text-gray-600 px-2 py-1 rounded-full text-xs font-semibold">{{ $surat->status }}</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-3 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('surat-keluar.show', $surat->id) }}" class="text-slate-500 hover:text-blue-600" title="Detail">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('surat-keluar.edit', $surat->id) }}" class="text-slate-500 hover:text-orange-600" title="Edit">
                                ...
                                <form id="delete-form-{{ $surat->id }}" action="{{ route('surat-keluar.destroy', $surat->id) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
                            </div>
                        </td>
                    </tr>
                    @empty
