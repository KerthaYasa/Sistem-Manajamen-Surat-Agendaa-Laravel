        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="text-slate-500 text-sm font-medium">No. Agenda: <span class="text-slate-900">{{ $suratKeluar->nomor_agenda }}</span></span>
                </div>
                <h1 class="text-2xl font-bold text-slate-900 leading-tight">Ke: {{ $suratKeluar->tujuan_surat }}</h1>
                <p class="text-slate-500 mt-1">Perihal: {{ $suratKeluar->perihal }}</p>
            </div>
            
            <div class="flex gap-3">
                <a href="{{ route('surat-keluar.edit', $suratKeluar->id) }}" class="inline-flex items-center px-4 py-2 bg-orange-50 border border-orange-200 rounded-lg font-medium text-sm text-orange-700 hover:bg-orange-100 transition shadow-sm">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                    Edit Data
                </a>
                <a href="{{ route('surat-keluar.index') }}" class="inline-flex items-center px-4 py-2 bg-slate-600 border border-transparent rounded-lg font-medium text-sm text-white hover:bg-slate-700 transition shadow-sm">
                    ...
                    <div class="p-6">
                        <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
                            ...
                            <div class="col-span-1">
                                ...
                                <dd class="mt-1 text-sm text-slate-900">{{ $suratKeluar->kategori->nama_kategori ?? '-' }}</dd>
                            </div>
                            <div class="col-span-1">
                                <dt class="text-xs font-medium text-slate-500 uppercase">Nomor Surat</dt>
                                <dd class="mt-1 text-sm text-slate-900">{{ $suratKeluar->nomor_surat }}</dd>
                            </div>
                            <div class="col-span-1">
                                <dt class="text-xs font-medium text-slate-500 uppercase">Tanggal Surat</dt>
                                <dd class="mt-1 text-sm text-slate-900">{{ date('d F Y', strtotime($suratKeluar->tanggal_surat)) }}</dd>
                            </div>
                            ...
                            <div class="col-span-1">
                                ...
                                <dd class="mt-1 text-sm text-slate-900 font-medium">
                                    ...
                                    {{ $suratKeluar->creator->name ?? 'System / Admin' }}
                                </dd>
                            </div>
                            ...
                            <div class="col-span-1">
                                ...
                                <dd class="mt-1 text-sm text-slate-900">
                                    {{ $suratKeluar->created_at->format('d F Y H:i') }}
                                </dd>
                            </div>
                            ...
                            <div class="col-span-1">
                                ...
                                <dd class="mt-1 text-sm text-slate-900">
                                    {{ $suratKeluar->updated_at->format('d F Y H:i') }}
                                </dd>
                            </div>
                            ...
                            <div class="col-span-1 md:col-span-2 mt-2">
                                ...
                                <dd class="text-sm text-slate-900 bg-slate-50 p-4 rounded-lg border border-slate-100 leading-relaxed">
                                    {{ $suratKeluar->isi_ringkas ?: 'Tidak ada ringkasan.' }}
                                </dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div class="p-6 flex flex-col items-center justify-center text-center h-64">
                        @if($suratKeluar->lampiran_file)
                            ...
                            <p class="text-sm font-medium text-slate-900 mb-1 truncate w-full px-4">
                                {{ Str::afterLast($suratKeluar->lampiran_file, '/') }}
                            </p>
                            <p class="text-xs text-slate-500 mb-4">Klik tombol di bawah untuk melihat file.</p>
                            
                            <a href="{{ asset('storage/' . $suratKeluar->lampiran_file) }}" target="_blank" class="w-full inline-flex justify-center items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition shadow-sm">
